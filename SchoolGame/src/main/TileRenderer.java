...
public class TileRenderer {
	...
	public TileRenderer(){
		...
		model = new Model(vertices, sprite, indices);
		
		for (int i = 0; i < Tile.tiles.length; i++){
			if (Tile.tiles[i] != null){
				if (!tile_sprites.containsKey(Tile.tiles[i].getSprite()))
					tile_sprites.put(Tile.tiles[i].getSprite(), new Sprite(Tile.tiles[i].getSprite() + ".png"));	
			}
		}
	}
	...
	public void renderTile(int id, int x, int y, Shader shader, Matrix4f world, Camera camera){
		shader.bind();
		if (tile_sprites.containsKey(Tile.tiles[id].getSprite())) 
			tile_sprites.get(Tile.tiles[id].getSprite()).bind(0);
